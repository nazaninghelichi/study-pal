...
async def send_buddy_reports() -> None:
    """Nightly job (now 8pm): emails every set accountability buddy a summary of
    their student's day, plus a link to confirm the real count and pick a
    sticker. Covers both web and Telegram users — both live in the same
    ...
    for user_id, buddy_email in pairs:
        try:
            ...
            token = await create_gift_token(user_id, report_date)
